---
name: ax-eval-check
description: "AX 레벨 체크. Claude Code 로그를 자동 분석하여 5축 점수 + 별점 레벨을 출력."
allowed-tools:
  - Bash
  - Read
  - Write
...
ax-analyst가 반환한 `ASSET_SCORE`와 `ASSET_DETAILS`를 기반으로 생성. 항상 출력 (정리력 점수가 있을 때).

```
📚 쌓고 있는 자산
• CLAUDE.md   ✅ {claude_md_lines}줄, {claude_md_sections}섹션  (또는 ❌ 없음)
• handoff.md  ✅ 있음  (또는 ❌ 없음)
• 슬래시 명령  {slash_cmd_defined}개 정의  (0이면 "아직 없음")
• 스킬        {skill_defined}개 정의  (0이면 "아직 없음")
• 참조 문서   {docs_volume}줄  (0이면 "아직 없음")

→ 정리력: {ASSET_SCORE_STARS} ({ASSET_SCORE}점)

💡 다음 자산 1순위: {ASSET_NEXT_TIP}
```

**`{ASSET_NEXT_TIP}` 선택 기준** (가장 효과 높은 미완성 자산 1개):
1. `handoff_present` == 0 → "handoff.md 만들기 — 다음 세션 시작이 훨씬 빨라져요"
2. `claude_md_lines` < 30 → "CLAUDE.md에 자주 쓰는 규칙 3줄 적어두기"
3. `slash_cmd_defined` == 0 → "반복 작업 1개를 /명령어로 정의해보세요"
4. `docs_volume` < 100 → "자주 참고하는 정보를 references/ 폴더에 정리해두세요"
5. `skill_defined` == 0 → "재사용 워크플로우를 스킬로 등록하면 팀과 공유할 수 있어요"
6. 모두 충족 → "훌륭해요! 팀원에게 이 자산을 공유해보세요"

**`{ASSET_SCORE_STARS}` 생성**: ASSET_SCORE 반올림 → ⭐ 개수 (1~5)

`ASSET_DETAILS`에 "프로젝트 경로 미확인"이 있으면:
```
📚 쌓고 있는 자산
• 프로젝트 경로를 확인할 수 없어 자산 스캔을 건너뛰었습니다.
  cwd 정보가 있는 세션이 쌓이면 자동으로 분석됩니다.
```

**`{HARNESS_BLOCK}` 규칙:**

ax-analyst가 반환한 `HARNESS_UNUSED`를 기반으로 생성. 미사용 기능이 있을 때만 출력. 빈 줄 1개로 위아래 구분.
...
`HARNESS_UNUSED`가 `없음`이면 이 블록 전체 생략 (빈 줄도 생략).

**기능 키 → 표시명 + 안내 메시지**:

| 기능 키 | 표시명 | 안내 메시지 |
|---------|--------|------------|
| `claude_md` | CLAUDE.md | 프로젝트 규칙을 한번 적어두면 매번 설명 안 해도 돼요 |
| `memory` | 메모리 | 자주 쓰는 맥락을 기억시키면 다음 대화에서 바로 이어가요 |
| `slash_cmd` | 슬래시 명령어 | /로 시작하는 명령어로 반복 작업을 한 번에 실행할 수 있어요 |
| `plan_mode` | 플랜 모드 | 복잡한 작업은 "계획 먼저" 모드로 실수를 줄일 수 있어요 |
| `sub_agent` | 서브에이전트 | 큰 작업을 쪼개서 나눠 맡기면 더 빠르고 정확해요 |
| `custom_skill` | 커스텀 스킬 | /handoff 같은 반복 워크플로우를 자동화할 수 있어요 |
| `rules` | Rules 파일 | 업무별 규칙 파일을 만들면 상황에 맞는 지시를 자동 적용해요 |
| `mcp` | MCP 연결 | 브라우저, 캘린더 등 외부 도구를 연결하면 Claude가 직접 조작해요 |
...
| 정리력 | "도구는 바뀌어도 자산은 남아요. CLAUDE.md, handoff.md 한 줄씩 쌓아가세요." |
